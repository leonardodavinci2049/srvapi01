import { ApiProperty } from '@nestjs/swagger';
import { Type } from 'class-transformer';
import { IsNotEmpty, IsNumber } from 'class-validator';

import { EndpointContextDto } from 'src/core/dto/endpoint-context.dto';

export class EntryItemUpdMainDto extends EndpointContextDto {
  @ApiProperty({
    description: 'Entry item movement ID (tbl_entrada_movimento)',
  })
  @IsNumber()
  @IsNotEmpty()
  @Type(() => Number)
  pe_item_movement_id!: number;

  @ApiProperty({ description: 'Purchased quantity' })
  @IsNumber()
  @IsNotEmpty()
  @Type(() => Number)
  pe_qt_comprada!: number;

  @ApiProperty({ description: 'Received quantity' })
  @IsNumber()
  @IsNotEmpty()
  @Type(() => Number)
  pe_qt_recebida!: number;

  @ApiProperty({ description: 'Unit value in real' })
  @IsNumber()
  @IsNotEmpty()
  @Type(() => Number)
  pe_vl_unit_real!: number;

  @ApiProperty({ description: 'Freight value in real' })
  @IsNumber()
  @IsNotEmpty()
  @Type(() => Number)
  pe_vl_frete_real!: number;

  @ApiProperty({ description: 'Invoice value' })
  @IsNumber()
  @IsNotEmpty()
  @Type(() => Number)
  pe_vl_nota!: number;
}
/*
Sample JSON for testing in body endpoint:
{
  "pe_app_id": 1,
  "pe_system_client_id": 1,
  "pe_store_id": 1,
  "pe_organization_id": "ORG001",
  "pe_user_id": "USER001",
  "pe_user_name": "John Doe",
  "pe_user_role": "admin",
  "pe_person_id": 29014,
  "pe_item_movement_id": 209971,
  "pe_qt_comprada": 5,
  "pe_qt_recebida": 5,
  "pe_vl_unit_real": 120.15,
  "pe_vl_frete_real": 10.1,
  "pe_vl_nota": 130.11
}
*/
