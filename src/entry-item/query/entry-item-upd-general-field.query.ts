...
interface EntryItemUpdGeneralFieldQueryResult {
  queryString: string;
  queryParams: [
    number,
    number,
...
export function EntryItemUpdGeneralFieldQuery(
  dataJsonDto: EntryItemUpdGeneralFieldDto,
): EntryItemUpdGeneralFieldQueryResult {
  ...
  const queryString = `call sp_entry_item_upd_general_field_v2(
        ?,
        ?,
        ?,
        ?,
  ...
  const queryParams: EntryItemUpdGeneralFieldQueryResult['queryParams'] = [
    olAppId,
    olSystemClientId,
    olStoreId,
    olOrganizationId,
